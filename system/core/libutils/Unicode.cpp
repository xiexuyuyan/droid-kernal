...
ssize_t utf32_to_utf8_length(const char32_t *src, size_t src_len)
{
    ...
    while (src < end) {
        ...
        if (SSIZE_MAX - char_len < ret) {
            ...
            LOG_E(TAG, "utf32_to_utf8_length: 0x534e4554, 37723026");
...
void utf32_to_utf8(const char32_t* src, size_t src_len, char* dst, size_t dst_len)
{
    ...
    while (cur_utf32 < end_utf32) {
        ...
        LOGF_ASSERT(dst_len < len, "%zu < %zu", dst_len, len);
        utf32_codepoint_to_utf8((uint8_t *)cur, *cur_utf32++, len);
        cur += len;
        dst_len -= len;
    }
    LOGF_ASSERT(dst_len < 1, "dst_len < 1: %zu < 1", dst_len);
    *cur = '\0';
}
...
void utf16_to_utf8(const char16_t* src, size_t src_len, char* dst, size_t dst_len)
{
    ...
    LOGF_ASSERT(dst_len < 1, "%zu < 1", dst_len);
    *cur = '\0';
}
...
ssize_t utf16_to_utf8_length(const char16_t *src, size_t src_len)
{
    ...
    while (src < end) {
        ...
        if (SSIZE_MAX - char_len < ret) {
            ...
            // returning from this function, so we cannot express how
            // long this string is in an ssize_t.
            LOG_E(TAG, "utf16_to_utf8_length: 0x534e4554, 37723026");
            // android_errorWriteLog(0x534e4554, "37723026");
            return -1;
        }
        ret += char_len;
...
ssize_t utf8_to_utf16_length(const uint8_t* u8str, size_t u8len, bool overreadIsFatal)
{
    ...
    while (u8cur < u8end) {
        ...
        if (u8cur + u8charLen - 1 >= u8end) {
            if (overreadIsFatal) {
                LOG_E(TAG, "utf8_to_utf16_length: Attempt to overread computing length of utf8 string");
            } else {
                return -1;
            }
        }
...
char16_t* utf8_to_utf16(const uint8_t* u8str, size_t u8len, char16_t* u16str, size_t u16len) {
    ...
    LOGF_ASSERT(u16len == 0 || u16len > SSIZE_MAX, "u16len is %zu", u16len);
...
char16_t* utf8_to_utf16_no_null_terminator(
    ...
    // A value > SSIZE_MAX is probably a negative value returned as an error and casted.
    LOGF_ASSERT(dstLen > SSIZE_MAX, "dstLen is %zu", dstLen);
